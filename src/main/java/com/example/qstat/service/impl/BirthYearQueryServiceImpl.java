...
public class BirthYearQueryServiceImpl extends ServiceImpl<PersonDataMapper, PersonData> implements BirthYearQueryService {
    ...
    public IPage<PersonData> queryByPage(BirthYearQuery query) {
        try {
            ...
            log.info("查询完成，返回{}条记录", result.getRecords().size());
            return result;
        } catch (Exception e) {
            log.error("出生年份查询失败", e);
            throw e;
        }
    }
    ...
    public Object statistics(BirthYearQuery query) {
        try {
            ...
            log.info("统计完成，stats：{}", stats);
            return stats;
        } catch (Exception e) {
            log.error("出生年份统计失败", e);
            throw e;
        }
    }
    ...
    private LambdaQueryWrapper<PersonData> buildWrapper(BirthYearQuery query) {
        ...
        // 添加年龄段条件
        if (query != null && !CollectionUtils.isEmpty(query.getAgeRanges())) {
            int currentYear = Year.now().getValue();
